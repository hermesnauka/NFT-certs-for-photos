...
## Layout
- `app01_java_nft/` — reference implementation (Java/Spring Boot backend + Solidity contracts + Next.js frontend). See its own `CLAUDE.md` for build/test commands and `docs/sdlc/` for architecture and design docs.
...
Each port shares the same product spec, API contract, data model, and Solidity contract as app01 (see that port's `docs/sdlc/`); only each port's own `CLAUDE.md` documents its actual tech stack and build/test commands.
...
- Each app subfolder owns its own `CLAUDE.md` with build/test/run commands specific to that app — don't re-paste the product spec into it; link back to `requirements.md` instead.
